...
const FilePreviewModal: React.FC<FilePreviewModalProps> = ({ fileId, isOpen, onRequestClose }) => {
  ...
  const renderPreview = () => {
    ...
    switch (fileData.type) {
      ...
      case 'image':
        return <img src={fileData.content} alt={fileData.filename} style={{ maxWidth: '100%', maxHeight: 600, display: 'block', margin: '0 auto' }} />;
      case 'unsupported':
...
// Inline file preview component for use outside a modal
export const FilePreviewInline: React.FC<{ fileId: string | null }> = ({ fileId }) => {
  const [fileData, setFileData] = useState<any>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const tableScrollRef = useRef<HTMLDivElement>(null);
