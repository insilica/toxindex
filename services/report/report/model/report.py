...
class Report:
    ...
    def create_report(project_id, file_path, user_id, title, description):
        ...
        ds.execute(
            "INSERT INTO reports (s3_reference, user_id, title, description) values (%s, %s, %s, %s)",
            params,
        )

        # Fetch and return the newly created report
        res = ds.find(
            "SELECT * from reports where s3_reference = %s AND user_id = %s",
            (s3_reference, user_id),
        )
        report = Report.from_row(res) if res else None
        ds.execute(
            "INSERT INTO project_reports (project_id, report_id) values (%s, %s)",
            (project_id, report.report_id),
        )
