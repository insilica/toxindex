...
def generate_report(project_id):
    ...
    report = Report.create_report(
        project_id, pdf_filename, 1, report_title, "This is a test report"
    )

    logging.info(
        f"Generated report for {inchi} and saved HTML to {html_filename} and PDF to {pdf_filename}"
    )
...
def download_report(project_id, path):
    return flask.send_from_directory(
        os.getcwd(), path, as_attachment=True, download_name="report.pdf"
    )
