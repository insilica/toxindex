...
from webserver.forms.registration_form import (
    RegistrationForm,
    ForgotPasswordForm,
    UpdatePasswordForm,
)
...
def generate_validation_link(user):
    ...
    ds.execute(
        "INSERT INTO user_link (user_id,link_token,expiration) values (%s,%s,%s)",
        params,
    )
    return link_token
...
def verify_message():
    return flask.render_template(
        "verify_message.html", email=flask.request.args.get("email")
    )


def verification(token):
    user_id = ds.find("SELECT user_id from user_link WHERE link_token=(%s)", (token,))[
        "user_id"
    ]
...
def send_password_reset(email):
    ...
    route = url_for(
        "reset_password", token=link, _external=True
    )  # Added token parameter and _external=True
...
def forgot_password():
    ...
    if flask.request.method == "GET":
        return flask.render_template(
            "forgot_password.html", form=form, sent_email=False
        )
