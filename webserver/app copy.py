...
# def api_task_create():
#     logging.info(f"[api_task_create] Called with {request.method}")
#     task_data = request.get_json()
#     message = task_data.get("message", "")
#     logging.info(f"[api_task_create] Received message: {message}")
#     title = generate_title(message)
#     user_id = flask_login.current_user.user_id
#     workflow_id = int(task_data.get("workflow", 1))
#     environment_id = task_data.get("environment_id")
#     sid = task_data.get("sid")
#     logging.info(f"[api_task_create] Creating task for user_id={user_id}, workflow_id={workflow_id}, environment_id={environment_id}, sid={sid}")
...
#     )
#     logging.info(f"[api_task_create] Created task: {task.to_dict() if task else None}")
...
#     if workflow_id == 1:
#         logging.info(f"[api_task_create] Executing ToxRAP (probra_task) for task_id={task.task_id}")
...
#         )
#     elif workflow_id == 2:
#         logging.info(f"[api_task_create] Executing ToxDirect (plain_openai_task) for task_id={task.task_id}")
...
#         )
#     elif workflow_id == 3:
#         logging.info(f"[api_task_create] Executing ToxJson (openai_json_schema_task) for task_id={task.task_id}")
...
#         )
#     else:
#         logging.info(f"[api_task_create] Executing default (probra_task) for task_id={task.task_id}")
...
# # LOGIN MANAGEMENT ===========================================================
# # Legacy server-rendered frontend routes (commented out for SPA migration)
...
# # API endpoints for SPA (keep these)
# app.route("/api/register", methods=["POST"])(csrf.exempt(login.api_register))
# app.route("/api/verify", methods=["GET"])(csrf.exempt(login.verify_message))
# app.route("/api/verification/<token>", methods=["GET", "POST"])(csrf.exempt(login.api_verification))
# app.route("/api/login", methods=["POST"])(csrf.exempt(login.api_login))
# app.route("/api/logout", methods=["POST"])(csrf.exempt(login.api_logout))
# app.route("/api/forgot_password", methods=["POST"])(csrf.exempt(login.api_forgot_password))
# app.route("/api/reset_password/<token>", methods=["POST"])(csrf.exempt(login.api_reset_password))
...
# # --- All environment, task, file, and chat session endpoints are now handled by Blueprints above ---
