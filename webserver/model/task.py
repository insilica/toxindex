...
class Task:
    def __init__(self, task_id, title, user_id, created_at=None):
    ...
    @staticmethod
    def from_row(row):
        return Task(row['task_id'], row['title'], row['user_id'], row['created_at'])

    @staticmethod
    def create_task(title, user_id):
        params = (title, user_id)
        ds.execute("INSERT INTO tasks (title, user_id) VALUES (%s, %s)", params)
